...
#define PYBIND11_MODULE(name, variable)                                                                                \
    static ::pybind11::module_::module_def PYBIND11_CONCAT(pybind11_module_def_, name) PYBIND11_MAYBE_UNUSED;          \
...
using is_function_pointer
    = bool_constant<std::is_pointer<T>::value && std::is_function<typename std::remove_pointer<T>::type>::value>;
...
using function_signature_t
    = conditional_t<std::is_function<F>::value,
                    F,
...
PYBIND11_RUNTIME_EXCEPTION(
    cast_error, PyExc_RuntimeError) /// Thrown when pybind11::cast or handle::call fail due to a type casting error
...
public:
    ...
    template <typename Container,
              typename
              = enable_if_t<std::is_convertible<decltype(*std::begin(std::declval<const Container &>())), T>::value>>
...
#    define PYBIND11_WORKAROUND_INCORRECT_MSVC_C4100(...) detail::silence_unused_warnings(__VA_ARGS__)
#else
...
#    define PYBIND11_WORKAROUND_INCORRECT_GCC_UNUSED_BUT_SET_PARAMETER(...) detail::silence_unused_warnings(__VA_ARGS__)
#else
